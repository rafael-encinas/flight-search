        ...
        for (Map item : list) { // we iterate for each one of the items of the list transforming it
            ...
            for(Itinerary itinerary : itineraries){
                for(Segment segment : itinerary.getSegments()){
                    ...
                    //Gets the city name for each flight departure and arrival using the findAirportsByKeyword function
                    List<Airport> departureResult =  findAirportsByKeyword(segment.getDeparture().getIataCode());
                    segment.getDeparture().setCityName(departureResult.get(0).getAddress().getCityName().toString());

                    List<Airport> arrivalResult =  findAirportsByKeyword(segment.getArrival().getIataCode());
                    segment.getArrival().setCityName(arrivalResult.get(0).getAddress().getCityName().toString());
                }
            }
